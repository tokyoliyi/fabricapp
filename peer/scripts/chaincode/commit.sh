...
peer lifecycle chaincode commit -o $ORDERER_HOSTPORT --ordererTLSHostnameOverride $ORDERER_HOST --channelID $APP_CHANNEL_NAME --name $CC_NAME --version $CC_PKG_VER --sequence $CC_SEQUENCE --tls true --cafile $TLS_CAFILE \
    --peerAddresses peer0.org1.fabric.test:7050 --tlsRootCertFiles $TLS_CA_FILE \
    --peerAddresses peer0.org2.fabric.test:7050 --tlsRootCertFiles $TLS_CA_FILE
